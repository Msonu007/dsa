    ...
    public void testMrunal() throws Exception {
     ...
     public void testRandomLevelFunctionality() {
         ...
         for (int i = 0; i < 1000; i++) {
             ...
             // Test that level is non-negative (indirectly tests Random initialization)
             Assert.assertTrue("Level should be non-negative", level >= 0);
             
             // Add the level to a set (tests increment and return)
             boolean isNewLevel = generatedLevels.add(level);
             if (isNewLevel && level > 0) {
                 // If a new level greater than 0 is added, it means the loop ran more than once
                 multipleIterations = true;
             }
         }

         // Ensure that the loop condition is being tested (the loop runs more than once)
         Assert.assertTrue("Loop should run multiple times generating different levels", multipleIterations);
         
         // Ensure that the method returns different levels, testing the increment and return statement
         Assert.assertTrue("Method should return different levels over multiple invocations", generatedLevels.size() > 1);
     }
     ...
     public void testRemoveByValueExistingValue() {
         ...
         KVPair<String,Rectangle> pair1 = new KVPair<String, Rectangle>("Value1",rec1);
         KVPair<String,Rectangle> pair2 = new KVPair<String, Rectangle>("Value2",rec2);
     ...
     public void testRemoveByValueEmptyList() {
         ...
         Rectangle rec3 = new Rectangle(0,0,20,20);
         KVPair<String,Rectangle> pair2 = new KVPair<String, Rectangle>("Value2",rec3);
         KVPair<String,Rectangle> removedPair = skipList.removeByValue(rec3);
         Assert.assertNull(removedPair);
     }
